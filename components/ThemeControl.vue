...
export default {
	...
	methods: {
		async onSubmit(event) {
			...
			if (this.id === null) {
				const res = await fetch('https://us-central1-tsg-live-5.cloudfunctions.net/tsglive/tahoiya/theme', {
					method: 'POST',
					mode: 'cors',
		...
		async onAddDummy(event) {
			...
			event.preventDefault();
			this.isSubmitting = true;
			const res = await fetch('https://us-central1-tsg-live-5.cloudfunctions.net/tsglive/tahoiya/meaning', {
				method: 'POST',
				mode: 'cors',
				body: new URLSearchParams({
					token: this.token,
		...
		async onToggleAcceptDummy(dummy, event) {
			...
			event.preventDefault();
			this.isSubmitting = true;
			const res = await fetch('https://us-central1-tsg-live-5.cloudfunctions.net/tsglive/tahoiya/meaning/accept', {
